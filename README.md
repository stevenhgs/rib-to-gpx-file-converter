...
This Python project converts .rib files produced by goggles of Recon Instruments to .gpx files. 
...
This file was generated by Snow2 goggles.
...
```bash
python src/main.py --in example/DAY90.rib --out output/DAY90_OUT.gpx --mode 1
``` 
...
Change the `mode_string` variable to either `'1'` or `'2'`.
...
Bytes have a decimal value between `0` and `255`. In these numbers a pattern can be seen: there are blocks of `255` every now and then.
...
goes under the minimum value `0` that the value of the byte at index `19` goes down by one and
that the value of the byte at index `20` goes back to `255`.
This could mean that these two bytes should be combined to get the real value this data represents.

The following values are gotten if these two bytes are combined like the bytes representing the elevation:  

<img src="./example/figures/height_and_descent.png" width="1000">

The first graph shows the values for the height and the second graph shows the combined values of the bytes at index `19` and index `20`.
As you can see the combined values of the bytes at index `19` and index `20` 
are inversely related to the values of the height.
